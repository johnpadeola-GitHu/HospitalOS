...
import { CATEGORIES, ARTICLES, articlesIn, getArticle, searchArticles } from "./helpContent";
import { PageHeader, Card, Pill, inputStyle, EmptyState } from "../../lib/ui";
  ...
  return (
    <div>
      ...
      {searching ? (
        ...
        <ArticleView article={article} onOpen={open} />
      ) : openCat ? (
        <div style={cardGrid}>
          {articlesIn(openCat).map((a) => <ArticleCard key={a.id} a={a} onOpen={open} />)}
        </div>
      ) : (
        <Landing onCat={setOpenCat} onOpen={open} />
      )}
    </div>
  );
}

function Landing({ onCat, onOpen }) {
  return (
    <>
      <div style={heroRow}>
        {[
...
function ArticleView({ article, onOpen }) {
  return (
